...
public class AuthGuardAspect {
    ...
    public void handleAuth(AuthGuard authGuard) {
        ...
        if (attributes != null) {
            ...
            System.out.println("Required Roles: " + String.join(", ", java.util.Arrays.stream(requiredRoles).map(UserRoles::name).toList()));

            if (userRoleHeader == null || !java.util.Arrays.stream(requiredRoles).anyMatch(role -> role.name().equalsIgnoreCase(userRoleHeader))) {
